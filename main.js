...
function scrollFunction() {
    ...
    if ($('.company').length) {
        ...
        if ((scrollTop >= offsetTop) && !jQuery('.company').data('scrolled')) {
            ...
            jQuery('.polo_1').each(function (i, self) {
                ...
                jQuery(self).html('<iframe src="https://danfoss-drives.com.ua/wp-content/themes/technosence/shape/' + id + '.html" frameborder="0" class="icon_svg" style="background: transparent"></iframe>');
            });
    ...
    if ($('#contacts').length) {
        ...
        if (scrollTop >= offsetTop && !$('#google-map-container').data('scrolled')) {
            jQuery.ajax({
                type: 'GET',
                url: 'https://danfoss-drives.com.ua/wp-admin/admin-ajax.php',
                data: { action: 'add_map' },
                dataType: 'html',
...
/* Form */
const formRef = document.querySelector('.block__form')


const handleSubmitForm = (e) => {
    e.preventDefault()
    const [name, surname, email, phone, comment] = e.target.children;
